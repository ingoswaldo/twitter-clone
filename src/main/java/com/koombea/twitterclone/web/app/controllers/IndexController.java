...
public class IndexController {
    ...
    public String index(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, Model model, Authentication authentication) {
        try {
            Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC,"createdAt"));
            NamesWithIdOnly user = userService.findNamesByUsername(authentication.getName());
            Page<PostSummary> posts = postService.getPaginatedPostsWithFollowedUserPostsByUserId(user.getId(), pageable);
            ...
            model.addAttribute("tweets", posts);
            return "index";
